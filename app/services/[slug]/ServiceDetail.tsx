...
function neighbours(slug: string): { prev: Service; next: Service } {
...
function sampleCase(slug: string) {
...
export function ServiceDetail({ slug }: Props) {
  ...
  const sample = sampleCase(slug);
  const sampleSummary = lang === "id" ? sample.summaryId : sample.summary;
  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-site px-6 md:px-10">
          <div className="grid grid-cols-12 gap-6">
            <div className="col-span-12 md:col-span-5">
              <SectionHeader number={service.number} label={t.common.brief} />
              <h2 className="mt-6 text-heading-1 text-balance">{sx.tagline}</h2>
            </div>
            <div className="col-span-12 md:col-span-6 md:col-start-7">
              <p className="text-body-l text-paper">{sx.body}</p>
              <p className="mt-6 text-body text-ink-5">{sx.body2}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="relative w-full border-y border-line py-24 md:py-32">
        <div className="mx-auto max-w-site px-6 md:px-10">
          <SectionHeader number={service.number} label={t.common.included} />
          <ul className="mt-12 grid grid-cols-12 gap-x-6 gap-y-6 md:mt-16">
            {sx.deliverables.map((d, i) => (
              <FadeIn
                as="li"
                key={d}
                delay={i * 0.06}
      ...
      <section className="relative w-full py-24 md:py-32">
        <div className="mx-auto max-w-site px-6 md:px-10">
          <SectionHeader number={service.number} label={t.common.sampleCase} />
          <Link
